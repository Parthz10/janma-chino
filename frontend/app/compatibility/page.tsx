...
import { CalendarDays, MapPin, Sparkles, Upload, HeartHandshake, User, ArrowRight } from 'lucide-react';
...
export default function CompatibilityPage() {
  ...
  const [qBrideNak, setQBrideNak] = useState(1);
  ...
  const getMoonDetailsFromLon = (moonLon: number) => {
    ...
    return { moonSign, moonNakIdx, moonPada, moon_sign: moonSign, moon_nakshatra_index: moonNakIdx, moon_pada: moonPada };
  };
  ...
  const handleQuickSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE}/compatibility/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          bride: { moon_nakshatra_index: qBrideNak, moon_pada: qBridePada },
          groom: { moon_nakshatra_index: qGroomNak, moon_pada: qGroomPada }
        })
      });
      ...
      const resultPayload = {
        ...
        brideChart: { ascendantSign: brideSign, placements: [{ name: 'चं', signNumber: brideSign, degree: 15.0 }] },
        groomChart: { ascendantSign: groomSign, placements: [{ name: 'चं', signNumber: groomSign, degree: 15.0 }] },
        koota: data.koota,
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bRes = await fetch(`${API_BASE}/chart`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const gRes = await fetch(`${API_BASE}/chart`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const brideMoon = getMoonDetailsFromLon(bData.positions.Moon.longitude);
      const groomMoon = getMoonDetailsFromLon(gData.positions.Moon.longitude);

      setLoadingMessage('Running Ashta Koota algorithms and performing Gemini relationship analysis...');
      const compRes = await fetch(`${API_BASE}/compatibility/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bride: brideMoon, groom: groomMoon })
      });
  ...
  const handlePhotoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bRes = await fetch(`${API_BASE}/vision/explain`, { method: 'POST', body: bFormData });
      ...
      const gRes = await fetch(`${API_BASE}/vision/explain`, { method: 'POST', body: gFormData });
      ...
      if (!bMoon || !gMoon) throw new Error('Could not read "Moon (चं)" planet positions from one or both charts. Please verify the photos have clear markings.');

      const brideMoon = getMoonDetailsFromLon((bMoon.signNumber - 1) * 30 + (bMoon.degree || 15.0));
      const groomMoon = getMoonDetailsFromLon((gMoon.signNumber - 1) * 30 + (gMoon.degree || 15.0));
      ...
      const formatParsedPlacements = (placements: any) => 
        (placements || []).map((p: any) => ({ name: nameMap[p.name] || p.name, signNumber: p.signNumber, degree: p.degree }));
      ...
      const resultPayload = {
        ...
        brideChart: { ascendantSign: bParsed.ascendantSign || 1, placements: formatParsedPlacements(bParsed.placements) },
        groomChart: { ascendantSign: gParsed.ascendantSign || 1, placements: formatParsedPlacements(gParsed.placements) },
        koota: compData.koota,
        synergy: compData.synergy,
        brideMoonDetails: brideMoon,
        groomMoonDetails: groomMoon
  ...
  const handleFileSelect = (side: 'bride' | 'groom', file: File | null) => {
    ...
    if (side === 'bride') { setBFile(file); setBPreview(url); } 
    else { setGFile(file); setGPreview(url); }
    setError(null);
  };
  ...
  return (
    <section className="grid min-h-[calc(100vh-3rem)] content-center gap-8 lg:grid-cols-[0.8fr_1.2fr] px-4">
      <div className="flex flex-col justify-center">
        ...
        <h1 className="text-4xl font-semibold leading-tight text-white sm:text-5xl">Understand relationship alignment.</h1>
        ...
        {error && (
          <div className="mt-6 rounded border border-rose-500/30 bg-rose-950/20 p-4 text-sm text-rose-300 max-w-xl">{error}</div>
        )}
      </div>
      ...
      <div className="relative rounded-xl border border-slate-700 bg-slate-900/60 p-6 shadow-2xl backdrop-blur-md">
          ...
          <form onSubmit={handleQuickSubmit} className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800 relative">
                <div className="absolute top-3 right-3 text-amber-500/20"><User className="h-10 w-10" /></div>
                ...
                <div className="space-y-4">
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Name</span>
                    <input value={qBrideName} onChange={(e) => setQBrideName(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm" placeholder="Bride Name" />
                  </label>
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Moon Nakshatra</span>
                    <select value={qBrideNak} onChange={(e) => setQBrideNak(Number(e.target.value))} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm">
                      {NAKSHATRAS_LIST.map((nakName, idx) => (<option key={idx} value={idx + 1}>{idx + 1}. {nakName}</option>))}
                    </select>
                  </label>
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Pada / Charana</span>
                    <select value={qBridePada} onChange={(e) => setQBridePada(Number(e.target.value))} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm">
                      {[1, 2, 3, 4].map(num => (<option key={num} value={num}>Pada {num}</option>))}
                    </select>
                  </label>
                </div>
              </div>
              <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800 relative">
                ...
                <div className="space-y-4">
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Name</span>
                    <input value={qGroomName} onChange={(e) => setQGroomName(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm" placeholder="Groom Name" />
                  </label>
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Moon Nakshatra</span>
                    <select value={qGroomNak} onChange={(e) => setQGroomNak(Number(e.target.value))} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm">
                      {NAKSHATRAS_LIST.map((nakName, idx) => (<option key={idx} value={idx + 1}>{idx + 1}. {nakName}</option>))}
                    </select>
                  </label>
                  <label className="grid gap-1.5 text-xs text-slate-400">
                    <span>Pada / Charana</span>
                    <select value={qGroomPada} onChange={(e) => setQGroomPada(Number(e.target.value))} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none focus:border-amber-400 text-white text-sm">
                      {[1, 2, 3, 4].map(num => (<option key={num} value={num}>Pada {num}</option>))}
                    </select>
                  </label>
                </div>
          ...
          <form onSubmit={handleManualSubmit} className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2 max-h-[480px] overflow-y-auto pr-1 [scrollbar-width:thin] [color-scheme:dark]">
              <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800">
                ...
                <div className="space-y-4">
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Name</span><input value={mBrideName} onChange={(e) => setMBrideName(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm" placeholder="Bride Name" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Date</span><input type="date" value={mBrideDate} onChange={(e) => setMBrideDate(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm [color-scheme:dark]" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Time</span><input type="time" value={mBrideTime} onChange={(e) => setMBrideTime(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm [color-scheme:dark]" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Place</span><input value={mBridePlace} onChange={(e) => setMBridePlace(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm" placeholder="Kathmandu, Nepal" /></label>
                  <div className="grid grid-cols-2 gap-2">
                    <label className="grid gap-1 text-xs text-slate-400"><span>Latitude</span><input value={mBrideLat} onChange={(e) => setMBrideLat(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-2 py-1.5 outline-none text-white text-xs" /></label>
                    <label className="grid gap-1 text-xs text-slate-400"><span>Longitude</span><input value={mBrideLon} onChange={(e) => setMBrideLon(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-2 py-1.5 outline-none text-white text-xs" /></label>
                  </div>
                </div>
              </div>
              <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800">
                ...
                <div className="space-y-4">
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Name</span><input value={mGroomName} onChange={(e) => setMGroomName(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm" placeholder="Groom Name" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Date</span><input type="date" value={mGroomDate} onChange={(e) => setMGroomDate(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm [color-scheme:dark]" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Time</span><input type="time" value={mGroomTime} onChange={(e) => setMGroomTime(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm [color-scheme:dark]" /></label>
                  <label className="grid gap-1.5 text-xs text-slate-400"><span>Birth Place</span><input value={mGroomPlace} onChange={(e) => setMGroomPlace(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-3 py-2 outline-none text-white text-sm" placeholder="Kathmandu, Nepal" /></label>
                  <div className="grid grid-cols-2 gap-2">
                    <label className="grid gap-1 text-xs text-slate-400"><span>Latitude</span><input value={mGroomLat} onChange={(e) => setMGroomLat(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-2 py-1.5 outline-none text-white text-xs" /></label>
                    <label className="grid gap-1 text-xs text-slate-400"><span>Longitude</span><input value={mGroomLon} onChange={(e) => setMGroomLon(e.target.value)} className="rounded border border-slate-800 bg-slate-950 px-2 py-1.5 outline-none text-white text-xs" /></label>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'upload' && (
          <form onSubmit={handlePhotoSubmit} className="space-y-6">
            <input type="file" ref={bFileInputRef} onChange={(e) => handleFileSelect('bride', e.target.files?.[0] || null)} accept="image/*" className="hidden" />
            <input type="file" ref={gFileInputRef} onChange={(e) => handleFileSelect('groom', e.target.files?.[0] || null)} accept="image/*" className="hidden" />
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="flex flex-col">
                <h3 className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-2">Bride Janma Chino</h3>
                {!bPreview ? (
                  <div onClick={() => bFileInputRef.current?.click()} className="flex-1 min-h-[160px] flex flex-col items-center justify-center border border-dashed border-slate-700 hover:border-amber-500/50 rounded-lg bg-slate-950/30 hover:bg-slate-950/60 cursor-pointer transition-all p-4 text-center">
                  ...
                  <div className="relative rounded border border-slate-800 overflow-hidden bg-slate-950 flex flex-col items-center p-3 gap-2 min-h-[160px] justify-center">
                    ...
                    <button type="button" onClick={() => { setBFile(null); setBPreview(null); }} className="text-[10px] font-bold text-rose-400 bg-rose-950/20 px-3 py-1 rounded border border-rose-900/30 hover:bg-rose-950/40">Remove</button>
                  </div>
                )}
              </div>
              <div className="flex flex-col">
                ...
                {!gPreview ? (
                  <div onClick={() => gFileInputRef.current?.click()} className="flex-1 min-h-[160px] flex flex-col items-center justify-center border border-dashed border-slate-700 hover:border-amber-500/50 rounded-lg bg-slate-950/30 hover:bg-slate-950/60 cursor-pointer transition-all p-4 text-center">
                    <Upload className="h-6 w-6 text-amber-400/80 mb-2" />
                    <span className="text-xs font-semibold text-white">Select or snap photo</span>
                    <span className="text-[10px] text-slate-400 mt-1 max-w-[150px]">Ensure central house layout is sharp</span>
                  </div>
                  ...
                  <div className="relative rounded border border-slate-800 overflow-hidden bg-slate-950 flex flex-col items-center p-3 gap-2 min-h-[160px] justify-center">
                    ...
                    <button type="button" onClick={() => { setGFile(null); setGPreview(null); }} className="text-[10px] font-bold text-rose-400 bg-rose-950/20 px-3 py-1 rounded border border-rose-900/30 hover:bg-rose-950/40">Remove</button>
                  </div>
                )}
              </div>
            </div>
            <button type="submit" disabled={!bFile || !gFile} className={`w-full inline-flex items-center justify-center gap-2 rounded px-5 py-3 font-semibold text-slate-950 transition-all shadow-md ${bFile && gFile ? 'bg-amber-500 hover:bg-amber-400 cursor-pointer' : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'}`}>
